...
static void home_gestures_create_notifications(void)
{
    ...
    lv_obj_t *bell = lv_label_create(panel);
    lv_obj_t *headline = lv_label_create(panel);
    lv_obj_t *state = lv_label_create(panel);

    home_gestures_add_header(panel, LV_SYMBOL_BELL, "NOTIFICATIONS",
                             home_gestures_notifications_back);

    lv_label_set_text(bell, LV_SYMBOL_BELL);
    lv_obj_align(bell, LV_ALIGN_TOP_MID, 0, 95);
    lv_obj_set_style_text_font(bell, &lv_font_montserrat_48,
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(bell, lv_color_hex(HOME_GESTURE_GOLD),
                                LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_label_set_text(headline, "NO NEW ITEMS");
    lv_obj_align(headline, LV_ALIGN_TOP_MID, 0, 158);
    lv_obj_set_style_text_font(headline, &lv_font_montserrat_20,
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(headline, lv_color_hex(HOME_GESTURE_TEXT),
                                LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_label_set_text(state, "WAITING");
    lv_obj_align(state, LV_ALIGN_TOP_MID, 0, 186);
    lv_obj_set_style_text_font(state, &lv_font_montserrat_12,
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(state, lv_color_hex(HOME_GESTURE_MUTED),
                                LV_PART_MAIN | LV_STATE_DEFAULT);

    home_gestures_add_row(panel, 234, LV_SYMBOL_BELL, "PHONE", "PENDING",
                          lv_color_hex(HOME_GESTURE_MUTED), NULL, NULL);
    home_gestures_add_row(panel, 298, LV_SYMBOL_REFRESH, "MQTT", "PENDING",
                          lv_color_hex(HOME_GESTURE_MUTED), NULL, NULL);
    home_gestures_add_row(panel, 362, LV_SYMBOL_BELL, "REMINDERS", "PENDING",
                          lv_color_hex(HOME_GESTURE_MUTED), NULL, NULL);
