...
| 蓝牙设置、PAN/NTP | 已接入 | 中文精简界面，显示实际连接状态；网络共享需在手机端开启 |
...
- 蜂窝菜单和控制中心均可进入，返回时回到各自入口。
- 仅保留蓝牙开关、手机连接状态、网络共享状态和查找/停止查找手机，全部中文显示。
- 手机连接区区分经典蓝牙连接与配套 App 连接；未连接 App 时禁用查找手机。
- 网络共享显示实际 PAN 网络连接状态，需要先在手机端开启蓝牙网络共享。
- 移除尚未实现的配对管理、通话、诊断和页面内 PAN 偏好开关。
- 页面按秒检查状态，仅在状态变化时更新控件；退出后释放页面和刷新定时器。
...
普通应用在打开时创建页面，切换动画结束后通过
`src/ui/generated/ui_screen_lifecycle.c` 统一调用各页销毁函数。蓝牙、设置、运动、
RGB、木鱼、计算器、日历、喝水提醒、番茄钟、相机、闹钟、指南针、天气详情、
位置详情、TF 文件管理和 GIF 展示页退出后释放控件，并清理页面定时器、待执行刷新
或解码资源。RGB、木鱼、TF 浏览器和录音页面不再开机预创建。

以下内容保留：

- 表盘、蜂窝菜单、音乐、控制中心和通知等主页导航所需页面。
- 音乐播放、闹钟、喝水提醒、番茄钟、RGB 灯效等后台服务，不因页面销毁而停止。
- 录音页被通知或提醒临时切走时保留，避免中断录音；用户主动返回时停止录音/回放，
  并在切换结束后释放页面和定时器。

蓝牙页创建前后及普通页面释放后输出 `ui_mem:`，包含 `free`、`used` 和 `peak`
系统堆字节数。反复进入同一页面再退出后，检查 `free` 是否回到相近水平；
...
确认手机已开启蓝牙网络共享、PAN profile 已连接，并且手机本身可以联网。蓝牙设置页
的“网络共享”只显示真实状态；可通过 FinSH 执行 `pan_cmd conn_pan` 进行调试。

### 点击蓝牙或切换应用后卡死

若日志出现 `lv_obj_allocate_spec_attr`、`lv_obj_class_create_obj` 分配失败，且
系统堆只剩几十字节，应先排查页面缓存累积。本次精简蓝牙页并让普通应用退出释放，
不应再随着逐个打开应用而永久保留所有页面树。烧录后可交替打开蓝牙、日历、计算器、
相机、TF 浏览器并返回，观察 `ui_mem:` 是否持续下降，同时确认后台音乐和提醒正常。

Linux/WSL 可使用 SDK 自带 LVGL 运行页面回归测试（需要 GCC、CMake 和 Ninja）：

```sh
cmake -S tests/ui_lifecycle -B /tmp/hsp-ui-lifecycle -G Ninja \
  -DSIFLI_SDK=/path/to/SiFli-SDK-V2.5 -DCMAKE_BUILD_TYPE=Debug
cmake --build /tmp/hsp-ui-lifecycle -j8
ctest --test-dir /tmp/hsp-ui-lifecycle --output-on-failure
```

测试覆盖蓝牙状态、开关、查找手机、两种入口返回、重复返回和反复创建/释放后的堆及
定时器数量，启用 AddressSanitizer/UndefinedBehaviorSanitizer，并输出 `bluetooth.ppm`
供检查中文布局。此测试模拟蓝牙服务，实际无线连接和后台音频仍需真机验证。
...
- PAN 网络共享需在手机端开启，手表页仅显示连接状态。
- OTA 当前仅升级本开发板的 `main.bin`；服务端证书、分区地址和清单格式必须与固件配置一致。
- 手表端尚未实现配对管理、通话和蓝牙诊断，蓝牙设置页已移除这些预留入口。
