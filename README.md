...
- 已接通音乐、RGB 灯、蓝牙、天气、位置、木鱼、闹钟、计算器、日历、喝水提醒、番茄钟、遥控拍照、指南针、设置和动态图片展示页面。
...
- 提供屏幕亮度、息屏时间、抬腕亮屏、音量、静音、震动和勿扰模式设置。
- 电池管理页显示实时电量、充电状态、电压和低电量模式开关。
- 低电量模式会限制屏幕亮度、缩短息屏时间并关闭抬腕亮屏。
- 系统信息页显示设备名称、SF32LB52/LVGL 版本和当前固件版本。
- 可从设置页进入关机与重启确认界面。
- 设置控件沿用闹钟页面的深灰色大圆角卡片、无边框布局和金黄色选中状态，适配小屏触控。
- 设置保存到 `/watch_settings.bin`，设备重启后自动恢复并重新应用。
...
- 最多保存 5 个闹钟，支持单独启用、关闭、编辑和删除。
- 使用小时与分钟滚轮选择时间，支持仅一次、每天、工作日和自定义星期重复。
- 闹钟列表顶部显示下一个有效闹钟及剩余时间。
- 到点后自动亮屏并周期震动，可选择关闭或稍后 5 分钟；5 分钟无操作会自动停止。
- 闹钟配置保存到 `/alarm_config.bin`，重启后仍然有效。

### 计算器

- 支持加、减、乘、除、百分比、小数和连续运算。
- 输入表达式时实时显示预计算结果，按下等号后突出最终结果。
- 退格键可以依次删除数字和待执行运算符，并处理除零等无效运算。

### 日历

- 提供 2000 至 2099 年的月历浏览，支持按钮或左右滑动切换月份。
- 高亮今天和当前选择日期，可一键返回当前月份。
- 日期详情显示星期、ISO 周数、农历干支、农历日期和节日信息。

### 喝水提醒

- 支持提醒总开关、开始时间、结束时间和 30 分钟至 6 小时的提醒间隔。
- 默认每次记录 200 ml、每日目标 1500 ml，并显示当日饮水进度和下一次提醒时间。
- 到点后亮屏、短震并显示提醒页，可选择“喝了”“稍后”或“跳过”。
- 20 秒无操作会自动结束本次提醒；闹钟、电源菜单等高优先级页面显示时会暂缓提醒。
- 结束时间到次日开始时间之间不会提醒，当日饮水量跨日自动清零。
- 提醒设置和当日饮水量保存到 `/water_config.bin`。

### 番茄钟

- 支持开始、暂停、继续、跳过当前阶段和结束确认。
- 默认采用 25 分钟专注、5 分钟短休息和 15 分钟长休息，时间均可在 1 至 99 分钟范围内调整。
- 专注与休息自动循环，每完成 4 个番茄进入长休息。
- 离开页面或息屏后继续计时，重新进入时按实际经过时间显示剩余时间。
- 支持阶段结束震动和预留提示音开关，记录今日番茄数量及累计专注时间并跨日清零。
- 时长、提醒选项和当日统计保存到 `/tomato_config.bin`。

### 遥控拍照与图片预览

- 通过蓝牙 HID 控制手机系统相机快门，不在配套 App 内创建独立拍照页面。
- 支持立即、3 秒和 5 秒延时拍照，倒计时每秒震动，触发快门时再次震动。
- 手表点击“预览”后向配套 App 请求手机最近一张照片，无需在手机端手动发送。
- 照片通过 BLE 分包传输并保存为 `/camera.jpg`，当前最大 16 KiB、最长边 160 px。
- 预览页支持放大、缩小和拖动查看。

### 指南针

- 使用板载 MMC56X3 三轴地磁传感器，进入页面后以 20 Hz 采样并实时更新方位。
- 显示航向角、八方位、磁场强度、校准进度和磁场干扰状态。
- 支持重置校准，按界面提示转动设备完成水平“8”字校准。
...
- 根据设置的息屏时间，在无操作后自动将 LCD 亮度降为 0，唤醒后恢复之前的亮度。
...
| `STATE` | 手表 -> 手机 | 查找手机、通知管理、拍照及照片预览请求 |
| `SYNC` | 手机 -> 手表 | 时间、位置、天气、通知、歌词、封面和照片数据同步 |
...
`image/app_grid/` 保存蜂窝菜单使用的应用图标，图标文件与 `app_grid_ui.c` 中的图片声明和应用枚举一一对应。

`image/number/` 保存主页数字、蓝牙状态、活动、通知和喝水图标；`image/number/tomoto/` 保存番茄钟及遥控拍照倒计时使用的数字图片。所有 `.png` 文件均为带透明通道的真实 RGBA PNG；三张 `.jpg` 为真机效果照片。
...
- 蜂窝菜单中的活动、电子书、骑行秒表、笔记、相册、录音和 SOS 当前只有图标与点击反馈，没有实际应用页面。
- 保险箱图标当前只进入动态 GIF 展示页，尚未实现文件加密、密码或数据保管功能。
- 位置页面是坐标信息和静态示意图，不包含地图瓦片、路线规划或导航。
- 蓝牙设置中的 PAN 开关只保存页面内偏好；底层 PAN/NTP 代码已存在，但尚未完整接入该开关。
- 连接新手机、移除配对、清除配对、通话和部分蓝牙诊断入口仍为预留。
- 活动累计和 RGB 参数仍主要保存在 RAM 中，重启后不会完整恢复。
- 番茄钟运行中的倒计时不会跨设备重启恢复；提示音接口已预留，但尚未接入实际音频资源。
- 遥控系统相机需要手机完成蓝牙 HID 配对；照片预览依赖 Android 配套 App、相册读取权限和 BLE 连接。
- 指南针采用水平面二维航向计算，倾斜佩戴时会产生误差，且每台设备需要真机校准与方向偏移验证。
- 歌词依赖手机通知读取权限、活跃媒体会话和网络；封面依赖播放器提供 artwork。

## Android 配套 App

Android 配套 App 位于独立项目 `hsp/`，负责连接与重连手表、双向查找、手机数据同步、通知转发、歌词与封面传输，以及最近照片的压缩和回传。Android 端有独立的实现说明，详见 [LCHSP_Watch_App](https://github.com/Gangan-307/LCHSP_Watch_App)。本 README 以手表固件为主，不展开 App 页面和权限细节。
...
src/ui/calculator/       计算器页面
src/ui/calendar/         公历、农历与节日页面
src/ui/camera/           遥控拍照与照片预览页面
src/ui/compass/          三轴地磁指南针页面
src/ui/details/          天气与位置详情页
src/ui/music/            音乐控制页
src/ui/settings/         系统设置、电池管理和系统信息页
...
| 固件版本 | 更新日期 | 主要修改 |
| --- | --- | --- |
| `0.3.0` | 2026-08-21 | 新增并完善闹钟、计算器、喝水提醒、番茄钟、日历、遥控拍照、图片预览、指南针和系统设置页面。 |
